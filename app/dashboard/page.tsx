...
export default async function DashboardPage() {
  ...
  try {
    ...
    const contactHref = `mailto:rohan.email.now@gmail.com?subject=${encodeURIComponent("Request API limit increase")}&body=${encodeURIComponent(`Hi team,\n\nPlease review and increase my monthly API limit.\n\nAccount: ${user.email ?? "unknown"}\nCurrent plan limit: ${usage.limit}/month\nCurrent usage this month: ${usage.used}\n\nThanks.`)}`;
      ...
      <div className="mx-auto flex w-full max-w-3xl flex-1 px-5 py-10 sm:px-8">
        <section className="card w-full p-6 sm:p-8">
          ...
          <p className="mt-3 text-sm text-[var(--muted)]">
            We could not load your dashboard right now. If this is production, check environment variables and database migrations.
          </p>
          <ul className="mt-4 list-disc space-y-2 pl-5 text-sm text-[var(--muted)]">
            <li>Confirm SUPABASE_SERVICE_ROLE_KEY is set in Vercel project settings.</li>
            <li>Run Supabase migrations so api_keys.user_id exists and policies are applied.</li>
            <li>Retry after redeploying with updated environment variables.</li>
          </ul>
        </section>
      </div>
    );
